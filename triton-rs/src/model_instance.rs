...
pub struct ModelInstance<ModelInstanceState, ModelState> {
...
impl<ModelInstanceState, ModelState> ModelInstance<ModelInstanceState, ModelState> {
    ...
    pub fn model(&self) -> Result<Model<ModelState>, Error> {
    ...
    pub fn state(&self) -> Result<&mut ModelInstanceState, Error> {
        let state = self.raw_state()?;
        if state.is_null() {
            return Err("Failed to get the state pointer".into());
        }

        let state: *mut ModelInstanceState = state as _;
        let state: &mut ModelInstanceState = unsafe { state.as_mut() }.unwrap();

        Ok(state)
    }

    pub fn replace_state(&self, new_state: Option<ModelInstanceState>)
            -> Result<Option<ModelInstanceState>, Error> {
        let old_state = self.raw_state()?;

        let new_state = match new_state {
            Some(new_state) => {
                let new_state = Box::new(new_state);
                Box::<ModelInstanceState>::into_raw(new_state) as *mut c_void
            },
            None => ptr::null_mut()
        };
        check_err(unsafe {
        ...
        let old_state = unsafe { Box::<ModelInstanceState>::from_raw(old_state) };
        Ok(Some(*old_state))
    }
